...
const GlassCard = ({ children, className = "", glow = false, glowColor = "indigo" }: { children: React.ReactNode, className?: string, glow?: boolean, glowColor?: "indigo" | "rose" | "emerald" }) => {
  const glowClasses = {
...
const Avatar = ({ src, name, status, role }: { src?: string, name: string, status?: 'online' | 'late' | 'offline', role?: string }) => (
  <div className="relative inline-flex items-center justify-center">
    <div className="w-11 h-11 rounded-full bg-gradient-to-br from-indigo-500/40 to-purple-500/40 border border-white/10 flex items-center justify-center text-white/90 font-medium shadow-inner overflow-hidden backdrop-blur-sm">
    ...
    {status === 'late' && <div className="absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full bg-rose-500 border-[2.5px] border-[#0B0F19] shadow-[0_0_8px_rgba(244,63,94,0.6)]" />}
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-[#050505] p-4 sm:p-8 font-sans selection:bg-indigo-500/30">
      ...
      <div className="relative w-[390px] h-[844px] bg-[#0A0C10] rounded-[56px] overflow-hidden shadow-[0_0_0_1px_rgba(255,255,255,0.05),0_40px_80px_rgba(0,0,0,0.8),0_0_120px_rgba(99,102,241,0.15)] ring-[8px] ring-[#1A1C23] flex flex-col text-slate-50">
        ...
        <div className="absolute -top-32 -right-32 w-[300px] h-[300px] bg-purple-600/10 blur-[100px] rounded-full pointer-events-none" />
        ...
        <header className="px-6 pt-16 pb-4 flex justify-between items-end z-10">
          <div>
            <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-white/5 border border-white/5 mb-3 backdrop-blur-md">
              ...
              <p className="text-white/60 text-[10px] font-semibold uppercase tracking-widest">Libby Story • Ridgeland, MS</p>
            </div>
          ...
          <div className="flex gap-2.5">
            ...
            <button className="w-10 h-10 rounded-full bg-white/5 border border-white/[0.08] flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 transition-all backdrop-blur-md relative">
              ...
              <div className="absolute top-2.5 right-2.5 w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)] border-2 border-[#12141D]" />
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto pb-40 scrollbar-hide px-6 pt-2 space-y-6 z-10">
          ...
          <div className="relative group cursor-pointer" style={{ transform: mounted ? 'translateY(0)' : 'translateY(20px)', opacity: mounted ? 1 : 0, transition: 'all 0.8s cubic-bezier(0.16, 1, 0.3, 1)' }}>
            <div className="absolute inset-0 bg-gradient-to-b from-indigo-500/20 to-purple-500/5 rounded-3xl blur-xl group-hover:blur-2xl transition-all duration-500 opacity-50" />
            <GlassCard glow className="p-6 border-indigo-500/20 bg-gradient-to-br from-[#12141D]/90 to-[#1A1C2A]/90">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-white/90 text-sm font-medium tracking-wide">Good morning, Libby</h2>
              </div>
              <p className="text-white/80 text-[15px] leading-relaxed mb-6 font-light">
                Store opened exactly on time. Sales are trending <span className="text-emerald-400 font-medium">12% above</span> yesterday. Here's what matters today.
              </p>
              
              {/* Action Item */}
              <div className="bg-gradient-to-br from-rose-500/[0.08] to-rose-500/[0.02] border border-rose-500/20 rounded-2xl p-4 flex gap-4 items-start relative overflow-hidden backdrop-blur-sm">
                <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-rose-400 to-rose-600" />
                <div className="mt-0.5 bg-rose-500/20 p-2 rounded-full border border-rose-500/30">
                  <AlertTriangle size={16} className="text-rose-400" />
                </div>
                <div className="flex-1">
                  <h3 className="text-rose-200 font-medium text-sm mb-1.5 flex items-center justify-between">
                    Summer is late
                    <span className="text-rose-400/50 text-[10px] uppercase font-bold tracking-wider">Urgent</span>
                  </h3>
                  <p className="text-rose-200/60 text-xs leading-relaxed mb-3">She hasn't clocked in. Her shift started 8 min ago.</p>
                  <div className="flex gap-2">
                    <button className="flex-1 py-2 rounded-xl bg-rose-500/20 text-rose-300 text-xs font-semibold hover:bg-rose-500/30 transition-all border border-rose-500/20 flex items-center justify-center gap-1.5 shadow-[0_0_15px_rgba(244,63,94,0.1)]">
                      <MessageSquare size={12} /> Message
                    </button>
                    <button className="px-4 py-2 rounded-xl bg-white/5 text-white/50 text-xs font-medium hover:bg-white/10 transition-all border border-white/5">
                      Ignore
                    </button>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 gap-4" style={{ transform: mounted ? 'translateY(0)' : 'translateY(20px)', opacity: mounted ? 1 : 0, transition: 'all 0.8s cubic-bezier(0.16, 1, 0.3, 1) 0.1s' }}>
            <GlassCard className="p-5 hover:bg-white/[0.08] transition-colors cursor-pointer group">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="px-2 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 text-[10px] font-bold tracking-wide">
                  +12%
                </div>
              </div>
              <div className="space-y-1">
                <p className="text-white/40 text-xs font-medium uppercase tracking-wider">Sales Pulse</p>
                <div className="flex items-baseline gap-1">
                  <span className="text-white text-2xl font-bold tracking-tight">$2,450</span>
                  <span className="text-white/30 text-sm font-medium">.00</span>
                </div>
              </div>
            </GlassCard>
            ...
            <GlassCard className="p-5 hover:bg-white/[0.08] transition-colors cursor-pointer group">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="px-2 py-1 rounded-lg bg-purple-500/10 text-purple-400 text-[10px] font-bold tracking-wide">
                  TOP 5%
                </div>
              </div>
              <div className="space-y-1">
                <p className="text-white/40 text-xs font-medium uppercase tracking-wider">Team Score</p>
                <div className="flex items-baseline gap-1">
                  <span className="text-white text-2xl font-bold tracking-tight">92</span>
          ...
          <div className="space-y-4" style={{ transform: mounted ? 'translateY(0)' : 'translateY(20px)', opacity: mounted ? 1 : 0, transition: 'all 0.8s cubic-bezier(0.16, 1, 0.3, 1) 0.2s' }}>
            <div className="flex justify-between items-end px-1">
              <h2 className="text-white/90 font-medium tracking-wide">On Floor Now</h2>
              <button className="text-indigo-400 text-xs font-medium hover:text-indigo-300 flex items-center gap-0.5 group">
            ...
            <GlassCard className="p-2">
              <div className="flex items-center justify-between p-3 rounded-2xl hover:bg-white/5 transition-colors cursor-pointer">
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <h4 className="text-white/90 text-sm font-medium tracking-wide">Taylor</h4>
                    <p className="text-white/40 text-[11px] mt-0.5">Manager • In since 8:45 AM</p>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <div className="px-2 py-1 rounded-md bg-white/5 border border-white/5 flex items-center gap-1.5">
                    ...
                    <span className="text-white/70 text-[10px] font-medium">4/5 tasks</span>
                  </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-2xl hover:bg-white/5 transition-colors cursor-pointer">
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <h4 className="text-white/90 text-sm font-medium tracking-wide">Sela</h4>
                    <p className="text-white/40 text-[11px] mt-0.5">Stylist • In since 9:00 AM</p>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <div className="px-2 py-1 rounded-md bg-white/5 border border-white/5 flex items-center gap-1.5">
                    <Clock size={10} className="text-white/50" />
                    <span className="text-white/50 text-[10px] font-medium">Break in 2h</span>
                  </div>
                  <MoreHorizontal size={14} className="text-white/20 mt-1" />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-2xl hover:bg-white/5 transition-colors cursor-pointer">
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <h4 className="text-white/60 text-sm font-medium tracking-wide">Sophia</h4>
                    <p className="text-white/30 text-[11px] mt-0.5">Stylist • Next up 1:00 PM</p>
                  </div>
                </div>
              </div>
            </GlassCard>
          ...
          <div className="space-y-4 pb-8" style={{ transform: mounted ? 'translateY(0)' : 'translateY(20px)', opacity: mounted ? 1 : 0, transition: 'all 0.8s cubic-bezier(0.16, 1, 0.3, 1) 0.3s' }}>
            <div className="flex justify-between items-end px-1">
              <h2 className="text-white/90 font-medium tracking-wide">Action Items</h2>
            ...
            <GlassCard className="p-4 flex items-center gap-4 hover:bg-white/[0.08] transition-colors cursor-pointer group">
              <div className="w-6 h-6 rounded-lg border-[1.5px] border-white/20 group-hover:border-indigo-400/50 flex-shrink-0 transition-colors" />
              <div className="flex-1">
                <h4 className="text-white/90 text-[15px] font-medium mb-1">Restock Summer Collection</h4>
                <p className="text-white/40 text-xs">Assigned to Sela • Due at 12:00 PM</p>
              </div>
            </GlassCard>
            <GlassCard className="p-4 flex items-center gap-4 opacity-50 bg-transparent border-dashed">
              <div className="w-6 h-6 rounded-lg border-[1.5px] border-emerald-500/50 bg-emerald-500/10 flex items-center justify-center flex-shrink-0">
                <CheckSquare size={12} className="text-emerald-400" />
              </div>
              <div className="flex-1">
                <h4 className="text-white/60 line-through text-[15px] font-medium mb-1">Morning Register Count</h4>
                <p className="text-white/30 text-xs">Completed by Taylor at 8:55 AM</p>
              </div>
            </GlassCard>
          </div>
        ...
        <div className="absolute bottom-[104px] right-6 z-[60]">
          <button className="relative group w-14 h-14 rounded-2xl bg-indigo-500 flex items-center justify-center shadow-[0_8px_32px_rgba(99,102,241,0.4),inset_0_2px_4px_rgba(255,255,255,0.3)] border border-indigo-400/50 transition-all hover:scale-105 active:scale-95 duration-300">
            {/* Outer rings pulse */}
            <div className="absolute inset-0 rounded-2xl border-[1.5px] border-indigo-400/30 scale-110 opacity-0 group-hover:opacity-100 group-hover:scale-125 transition-all duration-500" />
            <div className="absolute inset-0 rounded-2xl border border-indigo-400/10 scale-125 opacity-0 group-hover:opacity-100 group-hover:scale-[1.4] transition-all duration-700 delay-75" />
            
            {/* Inner glow */}
            <div className="absolute inset-0 rounded-2xl bg-indigo-400 opacity-0 group-hover:opacity-50 blur-lg transition-opacity duration-300" />
