...
export function registerSopLibraryRoutes(
  ...
  app.get('/api/sop/categories', isAuthenticated, asyncHandler(async (req: any, res) => {
    const storeId = await tryResolveStoreIdForUser(req.user.id);
  ...
  app.get('/api/sop/documents', isAuthenticated, asyncHandler(async (req: any, res) => {
  ...
  app.get('/api/sop/documents/:id', isAuthenticated, asyncHandler(async (req: any, res) => {
    const doc = await storage.getSopDocument(req.params.id);
    if (!doc) throw new AppError(404, "Document not found", "NOT_FOUND");
    const isAdmin = await resolvePermission(req.user.id, "admin.manage_all", storage);
    if (!isAdmin && !doc.isPublished) throw new AppError(404, "Document not found", "NOT_FOUND");
    res.json(doc);
  }));
  ...
  app.get('/api/sop/search', isAuthenticated, asyncHandler(async (req: any, res) => {
    const query = req.query.q as string;
    if (!query) throw new AppError(400, "Search query required", "MISSING_PARAM");
    const results = await storage.searchSopDocuments(query);
    res.json(results);
  }));

  app.get('/api/training/modules', isAuthenticated, asyncHandler(async (req: any, res) => {
    const storeId = await tryResolveStoreIdForUser(req.user.id);
  ...
  app.post('/api/training/progress', isAuthenticated, asyncHandler(async (req: any, res) => {
  ...
  app.get('/api/knowledge-base', isAuthenticated, asyncHandler(async (req: any, res) => {
    const storeId = await tryResolveStoreIdForUser(req.user.id);
    if (!storeId) throw new AppError(403, "Store access required", "FORBIDDEN");
  ...
  app.get('/api/knowledge-base/:id', isAuthenticated, asyncHandler(async (req: any, res) => {
    ...
    if (!article) throw new AppError(404, "Article not found", "NOT_FOUND");
