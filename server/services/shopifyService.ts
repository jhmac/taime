...
export class ShopifyService {
  ...
  async getSubscriptionStatus(subscriptionId: string): Promise<{
    ...
    const response = await this.makeGraphQLRequest(query, { id: subscriptionId });
  ...
  async cancelAppSubscription(subscriptionId: string): Promise<boolean> {
    ...
    const response = await this.makeGraphQLRequest(mutation, { id: subscriptionId });
    const result = response.data?.appSubscriptionCancel;

    if (result?.userErrors?.length > 0) {
      console.warn('[ShopifyService] Cancel subscription warnings:', result.userErrors);
      return false;
    }
