...
export function redactSensitiveFields<T extends Record<string, any>>(user: T): Omit<T, typeof SENSITIVE_FIELDS[number]> {
  const copy = { ...user };
...
export function redactSensitiveFieldsFromArray<T extends Record<string, any>>(
  users: T[],
): Omit<T, typeof SENSITIVE_FIELDS[number]>[] {
  return users.map(redactSensitiveFields);
}

export async function canSeeSensitiveData(
