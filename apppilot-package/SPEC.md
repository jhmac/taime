# AppPilot Specification

**Version**: 0.3.5
**Last Updated**: February 20, 2026

AppPilot is an autonomous AI agent framework that embeds inside any Node.js/Express application. It monitors, analyzes, and autonomously improves the host app using Claude AI — without human intervention. It includes ELON, a strategic constraint solver that identifies and fixes the single biggest thing blocking an app from reaching its goals.
...
AppPilot initializes as Express middleware, attaching itself to the host app's Express instance. It provides:
...
**Initialization** (`initAppPilot(app, config)`):
...
5. Optionally mounts the admin dashboard at `/apppilot/dashboard`.
6. Returns `{ context, memory, identity, middleware }` for programmatic access.

**Exports**: `initAppPilot`, `runHeartbeat`, `Orchestrator`, `MemoryStore`, `IdentityProtection`, `loadContext`, `buildSystemPrompt`, `parseHeartbeatConfig`, `runElonCycle`, `runElonLoop`, `evaluateConstraint`, `getElonStatus`, `crawlSite`, `verifyCrawl`.
...
AppPilot's behavior is defined by a set of Markdown files in the project root. Each file serves a specific purpose and together they form the agent's "personality," permissions, and priorities.
...
All files use optional YAML frontmatter (parsed by `gray-matter`) and Markdown content. Templates with `-template.md` suffix are scaffolded via `npx apppilot init`.
...
### OwnerVerification
- Authenticates dashboard requests using a shared secret (`APPPILOT_INTERNAL_KEY`).
...
- Persists checksums to `.apppilot/identity-checksums.json`.
...
### OutputValidator
- Blocks file writes to identity files (`SOUL.md`, `AGENTS.md`, etc.), `.env` files, `package.json`, `node_modules/`, and `apppilot/` internals.
...
The MemoryStore provides persistent storage for all AppPilot operations, using the filesystem (`.apppilot/` directory) rather than a database.
...
AppPilot delegates specialized tasks to purpose-built subagents, each with its own prompt template and model tier.
...
Reflects on AppPilot's own performance and proposes improvements.
...
### File Operations
- **`backup(filePath)`**: Copies the file to `.apppilot/backups/` with a timestamp suffix. Returns the backup path.
...
- **Authenticated crawling**: Checks for a stored session (`.apppilot/crawler-session.json`). If valid, sets the `__session` cookie and verifies authentication via `window.Clerk.user` or `/api/users/me`.
...
### Session Management
- **Storage**: Sessions are persisted in `.apppilot/crawler-session.json` with `sessionToken`, `userId`, `userEmail`, and `expiresAt`.
...
- Results saved to `.apppilot/last-health-check.json` for caching.
...
### Scenario Definition
Scenarios are JSON files in `.apppilot/scenarios/` or hardcoded defaults:
```json
{
  "id": "shopify-connect",
...
- Results saved to `.apppilot/last-scenario-results.json`.
...
- Maintains a registry in `.apppilot/regression-tracker.json`.
...
Saved to `.apppilot/dependency-index.json` and refreshed each ELON cycle.
...
A full-featured web dashboard mounted at `/apppilot/dashboard` for monitoring and controlling all AppPilot/ELON operations.

### Authentication
- Protected by `APPPILOT_INTERNAL_KEY` via query parameter or `x-apppilot-key` header.
...
| **Activity Feed** | Real-time timestamped log of all AppPilot actions (specs executed, errors found, ELON progress). |
...
|-------|--------|-------------|
| `/apppilot/api/status` | GET | System status, metrics, crawler auth |
| `/apppilot/api/feed` | GET | Activity feed entries |
| `/apppilot/api/errors` | GET | Known errors list |
| `/apppilot/api/metrics` | GET | Detailed metrics |
| `/apppilot/api/queue` | GET | Pending spec queue |
| `/apppilot/api/queue/:id/approve` | POST | Approve a spec |
| `/apppilot/api/queue/:id/reject` | POST | Reject a spec |
| `/apppilot/api/security` | GET | Identity file integrity status |
| `/apppilot/api/security/acknowledge` | POST | Acknowledge identity changes |
| `/apppilot/api/elon/run` | POST | Start a single ELON cycle |
| `/apppilot/api/elon/evaluate` | POST | Evaluate current constraint |
| `/apppilot/api/elon/status` | GET | ELON status and progress |
| `/apppilot/api/elon/settings` | GET/POST | ELON settings (budget, crawl mode) |
| `/apppilot/api/elon/specs` | GET | ELON-generated specs |
| `/apppilot/api/elon/specs/:id/approve` | POST | Approve an ELON spec |
| `/apppilot/api/elon/specs/:id/reject` | POST | Reject an ELON spec |
| `/apppilot/api/elon/specs/approve-all` | POST | Approve all ELON specs |
| `/apppilot/api/elon/fix-all` | POST | Start Fix-All mode |
| `/apppilot/api/elon/report` | GET | Latest ELON report |
| `/apppilot/api/elon/progress` | GET | ELON execution progress |
| `/apppilot/api/integration-health` | GET | Integration health status |
| `/apppilot/api/scenario-results` | GET | Scenario test results |
| `/apppilot/api/run-scenarios` | POST | Trigger scenario tests |
| `/apppilot/api/regressions` | GET | Regression tracker data |
| `/apppilot/api/dev-mode` | GET/POST | Dev mode status/toggle |
| `/apppilot/api/dependency-index` | GET | Dependency index |
| `/apppilot/crawler-login` | GET | Crawler auth popup page |
| `/apppilot/api/crawler-session` | GET/POST/DELETE | Manage crawler session |

---

## 19. CLI Commands

**Files**: `bin/apppilot.js`, `bin/heartbeat.js`, `bin/elon.js`, `bin/crawl.js`, `bin/continuous.js`

| Command | Description |
|---------|-------------|
| `npx apppilot init` | Scaffolds identity files and data directories. Creates SOUL.md, AGENTS.md, GOALS.md, etc. from templates. Initializes `.apppilot/` with subdirectories, checksums, and `.gitignore` entries. |
| `npx apppilot status` | Shows the current state: which identity files exist, spec queue counts, known errors count, checksum protection status. |
| `npx apppilot heartbeat` | Runs a single heartbeat cycle (monitoring + autonomous fixes). Supports `--dry-run`. |
| `npx apppilot-elon` | Runs a single ELON constraint-solving cycle. Identifies the #1 limiting factor and creates specs. |
| `npx apppilot-crawl` | Crawls the live site with Playwright and reports errors. |
| `npx apppilot-continuous` | Runs the continuous improvement loop (ELON + heartbeat cycling). |

### Environment Variables
| Variable | Required | Description |
|----------|----------|-------------|
| `APPPILOT_ANTHROPIC_KEY` or `ANTHROPIC_API_KEY` | Yes | Claude API key for all AI operations |
| `ANTHROPIC_BASE_URL` | No | Custom API endpoint (for proxies) |
| `APPPILOT_INTERNAL_KEY` | No | Shared secret for dashboard authentication |
