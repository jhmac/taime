...
class OwnerVerification {
  constructor(config = {}) {
    ...
    this.internalKey = config.internalKey || process.env.APPPILOT_INTERNAL_KEY;
  }

  verifyRequest(req) {
    const key = req.headers['x-apppilot-key'] || req.query.key;
  ...
  async logOwnerAction(action, details, dataDir) {
    ...
    if (dataDir) {
      ...
      try {
        ...
        console.error(`[AppPilot] Failed to log owner action: ${err.message}`);
      }
    }
...
class IdentityProtection {
  constructor(projectRoot = '.', dataDir) {
    ...
    this.dataDir = dataDir || path.join(projectRoot, '.apppilot');
  ...
  _persistChecksums() {
    try {
      ...
      console.error(`[AppPilot] Failed to persist identity checksums: ${err.message}`);
    }
  }
