...
<head>
  ...
  <title>AppPilot Dashboard</title>
  <style>
...
function apppilotMiddleware(options = {}) {
  const {
    ...
    internalKey = process.env.APPPILOT_INTERNAL_KEY,
    context = null,
    memoryStore = null,
  } = options;
  ...
  function mainMiddleware(req, res, next) {
    ...
    if (enableDashboard && req.method === 'GET' && req.path === dashboardPath) {
      if (internalKey) {
        const providedKey = req.headers['x-apppilot-key'] || req.query.key;
