...
function init() {
  ...
  const knownErrorsPath = path.join(TARGET_DIR, '.sneebly', 'known-errors.json');
  if (!fs.existsSync(knownErrorsPath)) {
    fs.writeFileSync(knownErrorsPath, JSON.stringify({ errors: [] }, null, 2));
  }
  const metricsPath = path.join(TARGET_DIR, '.sneebly', 'metrics.json');
...
function status() {
  ...
  const sneeblyDir = path.join(TARGET_DIR, '.sneebly');
  console.log(`\nData directory: ${fs.existsSync(sneeblyDir) ? 'exists' : 'not found'}`);

  const queueDirs = ['queue/pending', 'approved-queue', 'completed', 'failed'];
  for (const dir of queueDirs) {
    const dirPath = path.join(sneeblyDir, dir);
  ...
  if (fs.existsSync(path.join(sneeblyDir, 'known-errors.json'))) {
    try {
      const errors = JSON.parse(fs.readFileSync(path.join(sneeblyDir, 'known-errors.json'), 'utf-8'));
      console.log(`\nKnown errors: ${errors.errors.length}`);
    } catch {}
  }

  const checksumPath = path.join(sneeblyDir, 'identity-checksums.json');
