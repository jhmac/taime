...
- **SOP Library**: Structured operating procedures with templates (versioned, per-location, role-assignable), ordered steps (action/verification/photo/decision/timer types), execution tracking, and step-level completions with checkpoints, photo evidence, manager sign-off, and skip reasons. Tables: `sop_templates`, `sop_steps`, `sop_executions`, `sop_step_completions`. Note: `store_id` references `work_locations(id)` since that represents physical store locations. Activity logs are created on execution completion/abandonment. API routes in `server/routes/sops.ts` (10 endpoints): CRUD templates with versioning, start/update/list executions, complete/skip steps with auto-completion detection, manager sign-off on checkpoints. WebSocket events: `execution_started`, `step_completed`, `execution_completed`, `sign_off_requested`, `sign_off_completed`. All writes wrapped in Drizzle transactions.
