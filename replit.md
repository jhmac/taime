...
- **SOP Training Mode & Visual Walkthroughs**: Enhanced SOP content for new hires and visual learners. `sop_steps` has `training_video_url`, `training_photo_urls` (jsonb), `training_video_thumbnail`. `sop_templates` has `walkthrough_video_url`, `is_training_priority`. SOPBuilder includes per-step training media sections (photo upload with compression, video URL), training priority toggle, and walkthrough video URL field. SOPExecution training mode shows photo gallery (swipeable thumbnails with enlargement), video player (YouTube embed or native), "Why We Do This" callout, "Got it?" confirmation before advancing, 5-second delay when media exists, "I have a question" button linking to Ask MAinager. Auto-training mode activates for employees with <3 completions of an SOP. Training Hub page at `/sops/training` shows all training-priority SOPs grouped by mastery level (beginner/practicing/mastered) with progress bar. Training card on Associate Dashboard. API: `GET /api/sops/templates/training-priority`, `PUT /api/sops/templates/:id/steps/:stepId/media`, `GET /api/sops/executions/completion-count`.
...
- **Lean Board**: Team-level improvement tracking board (`server/services/leanBoard.ts`, `client/src/pages/LeanBoard.tsx`). `lean_board_snapshots` table stores daily metrics (improvements submitted, videos uploaded, kudos given, SOP completion rate, issues opened/resolved, team participation rate, active improvement streaks). Daily snapshot cron at 11 PM, weekly AI summary cron on Sundays. Pattern detection over 30-day rolling window (velocity, participation concentration, issue-to-improvement ratio, SOP compliance trend). Frontend: vibrant team scoreboard with metric cards, trend mini-charts, pattern cards, streak highlights, and weekly AI summary. Compact `LeanBoardCard` on all dashboards. API: `GET /api/lean-board`, `GET /api/lean-board/history`, `POST /api/lean-board/generate-snapshot`, `POST /api/lean-board/generate-summary`. Design principle: shows TEAM aggregates only, never individual rankings.
- **SOP Evolution System**: AI-powered revision proposals from employee feedback and analytics (`server/services/sopEvolution.ts`, `server/routes/sopEvolution.ts`). `sop_revision_proposals` table stores proposals with source tracking, proposal types, and review lifecycle. `generateRevisionProposals()` gathers "What Bugged You" process debriefs + SOP insights, matches signals to SOPs, calls Claude to generate concrete revision proposals (add/remove/modify/split steps). Sunday 7AM cron via `startSOPEvolutionCron()`. API: `GET /api/sops/revisions` (filtered by status/template), `PUT /api/sops/revisions/:id` (approve/reject with owner-only auth), `GET /api/sops/revisions/stats`, `POST /api/sops/revisions/generate`. Frontend: `SOPRevisions.tsx` page at `/sops/revisions` with proposals grouped by SOP, expandable rationale, approve-and-edit flow. `SOPRevisionCard` on Manager/Owner dashboards. `PendingRevisionsBadge` on SOP Detail page header. Design: suggestions not demands, anonymized feedback, owner always has final say.
- **SOP Intelligence Layer**: AI-powered SOP execution analysis (`server/services/sopIntelligence.ts`, `server/routes/sopIntelligence.ts`). `sop_insights` table stores AI-generated insights with type/severity/status lifecycle. `analyzeSOP()` computes 30-day metrics: overall (completion rate, avg time, steps skipped), step-level (avg/median time, skip rate, photo compliance, friction flags), employee patterns, and 4-week trends. `generateSOPInsights()` runs analysis for all active templates, calls Claude to produce actionable insights (friction, skip patterns, time trends, training gaps, optimizations). Sunday 6AM cron via `startSOPInsightsCron()`. API: `GET /api/sops/insights` (filtered by severity/template/status, sorted by severity), `PUT /api/sops/insights/:id/acknowledge`, `GET /api/sops/analytics/:templateId`, `POST /api/sops/insights/generate`. Frontend: expandable Analytics section on `SOPDetail.tsx` (step performance, team performance, weekly trends), `SOPInsightsCard` on Manager/Owner dashboards. Auth: manager/admin/owner only. Store-scoped for data isolation.
